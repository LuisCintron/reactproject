...
class Blog extends Component {
    ...
    getBlogItems() {
        ...
        axios
            .get(
                `https://luiscintron.devcamp.space/portfolio/portfolio_blogs?page=${this
                    .state.currentPage}`,
                {
    ...
    render() {
        const blogRecords = this.state.blogItems.map(blogItem => {
            if (this.props.loggedInStatus === 'LOGGED_IN') {
                return (
                    <div key={blogItem.id} className='admin-blog-wrapper'><BlogItem blogItem={blogItem} />
                        <a onClick={() => this.handleDeleteClick(blogItem)}><FontAwesomeIcon icon='trash' /> </a></div>
                )
            } else {
